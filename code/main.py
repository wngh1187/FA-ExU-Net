...
import arguments_large
import arguments_medium
import arguments_small
import trainers.train as train
import data.data_loader as data
from data.voxceleb1 import VoxCeleb1
from data.voxceleb2 import VoxCeleb2
from models.FA_ExU_Net import Model
...
def run(process_id, args, experiemt_args):
    ...
    trainer.vox2 = VoxCeleb2(
        args['path_vox2_train']
    )
    args['num_speaker'] = len(trainer.vox2.train_speakers)

    # data loader
    loaders = data.get_loaders(args, trainer.vox2, trainer.vox1)
    trainer.train_set = loaders[0]
    trainer.train_set_sampler = loaders[1]
    trainer.train_loader = loaders[2]
    trainer.enrollment_set = loaders[3]
    trainer.enrollment_loader = loaders[4]
    ...
    criterion['classification_loss'] = classification_loss_function(args['code_dim'], args['num_speaker'], args).to(args['device']) 
    criterion_params = list(criterion['classification_loss'].parameters())
    criterion['classification_loss'] = nn.parallel.DistributedDataParallel(criterion['classification_loss'], device_ids=[args['device']], find_unused_parameters=False)
    ...
    if args['do_train_code_enhancement']: 
        ...
        criterion['code_enhancement_loss'] = nn.parallel.DistributedDataParallel(criterion['code_enhancement_loss'], device_ids=[args['device']], find_unused_parameters=False)
    ...
    if args['learning_rate_scheduler'] == 'cosine':
        ...
        trainer.lr_scheduler = torch.optim.lr_scheduler.StepLR(
            trainer.optimizer, 3, gamma=0.95)
...
if __name__ == '__main__':
    # get arguments
    args, system_args, experiment_args = arguments_large.get_args()
    # args, system_args, experiment_args = arguments_medium.get_args()
    # args, system_args, experiment_args = arguments_small.get_args()
